...
import { asyncHandler, globalError } from '../../src/services/asyncHandler.js'
...
export const appRouter = (app) => {
    ...
    app.get('/EV/CookiesId', asyncHandler(async (req, res) => {
        ...
        if (visited.length == 0) {
            ...
            if (AddEV) {
                res.status(201).json({ message: "added successfully", AddEV })
            } else {
            ...
            if (updateEV) {
                res.status(201).json({ message: "updated", updateEV })
            } else {
